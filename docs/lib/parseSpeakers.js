 ...
 * ```
 */
export function parseSpeakers(vttContent) {
 ...
 */
function parseCueTiming(timingLine) {
 ...
 */
function collectCueText(lines, startIndex) {
	const textLines = [];
	let i = startIndex;
	while (i < lines.length) {
		const line = lines[i].trim();
 ...
 */
function skipCueLines(lines, startIndex) {
	let i = startIndex + 1;
	while (i < lines.length) {
		const line = lines[i].trim();
 ...
 */
function extractSpeakerFromVoiceTag(text) {
...
//# sourceMappingURL=parseSpeakers.js.map
